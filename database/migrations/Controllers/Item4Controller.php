...
{
    public function Item4(Request $request)
    {
    ...
    //  $data = $ItemService->checkDate();
        ...
        $ItemService = new ItemService();
        $title = config('config.FIFTH_NAME');
        list($json,$n) = $ItemService->rakutenApi(config('config.FIFTH_NAME'),config('app.appId'),config('app.afi'));
     ...
     //}
      ...
      list($items,$n) = $ItemService->sortItem($request->sort,config('config.FIFTH'));
