...
{
    public function Item2(Request $request)
    {
      ...
      // if($data['dtNow'] !== $data['latestDateTime']){
        $ItemService = new ItemService();
        $title = config('config.THIRD_NAME');
        list($json,$n) = $ItemService->rakutenApi(config('config.THIRD_NAME'),config('app.appId'),config('app.afi'));
      ...
      list($items,$n) = $ItemService->sortItem($request->sort,config('config.THIRD'));
      return view('item2')->with([
        'items' => $items,
        'n'     => $n,
      ]);
    }
