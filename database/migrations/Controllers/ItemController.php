...
class ItemController extends Controller
{
    ...
    //  $data = $ItemService->checkDate();
      ...
      // if($data['dtNow'] !== $data['latestDateTime']){
        $ItemService = new ItemService();
        $title = config('config.SECOND_NAME');
        list($json,$n) = $ItemService->rakutenApi(config('config.SECOND_NAME'),config('app.appId'),config('app.afi'));
      ...
      //}
      list($items,$n) = $ItemService->sortItem($request->sort,config('config.SECOND'));
      return view('item1')->with([
        'items' => $items,
        'n'     => $n,
      ]);
    }
